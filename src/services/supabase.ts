...
// Helper: Get user's Smartlead credentials from profile
export async function getUserSmartleadCredentials(userId: string) {
  ...
  if (!data?.smartlead_api_key || !data?.smartlead_email_account_id) {
    throw new Error(`User ${userId} has not connected their Smartlead account`);
  }
...
// Helper: Fetch enrollments ready to be processed with user's Smartlead credentials
