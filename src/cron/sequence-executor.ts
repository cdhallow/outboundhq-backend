...
async function processEnrollment(enrollment: any) {
  ...
  if (currentStep.step_type === 'email') {
    await processEmailStep(enrollment, contact, sequence, currentStep);
...
async function processEmailStep(enrollment: any, contact: any, sequence: any, step: any) {
  ...
  try {
    // Add contact to Smartlead campaign
    const leadId = await addLeadToCampaign(campaignId, {
      email: contact.email,
